...
if ($peut_demander && isset($_POST['creer_demande'])) {
    ...
    if (empty($raison)) {
        ...
        $stmt = mysqli_prepare($conn, "INSERT INTO demandes_acces (id_utilisateur, type_demande, role_demande, permission_demande, raison) VALUES (?, ?, ?, ?, ?)");
        mysqli_stmt_bind_param($stmt, "issss", $id_utilisateur, $type_demande, $role_demande, $permission_demande, $raison);
        
        if (mysqli_stmt_execute($stmt)) {
            $message = "✅ Votre demande a été envoyée avec succès. Un administrateur va l'examiner.";
        } else {
            $message = "❌ Erreur lors de l'envoi de la demande : " . mysqli_error($conn);
        }
...
if ($peut_approuver && isset($_POST['traiter_demande'])) {
    ...
    $stmt = mysqli_prepare($conn, "SELECT * FROM demandes_acces WHERE id = ? AND statut = 'en_attente'");
    ...
    if ($demande) {
        if ($action === 'approuver') {
            ...
            if ($demande['type_demande'] === 'role' && !empty($demande['role_demande'])) {
                ...
                if ($nouveau_role !== 'admin') {
                    ...
                    mysqli_stmt_bind_param($stmt_old, "i", $demande['id_utilisateur']);
                    mysqli_stmt_execute($stmt_old);
                    $result_old = mysqli_stmt_get_result($stmt_old);
                    $ancien = mysqli_fetch_assoc($result_old);
                    mysqli_stmt_close($stmt_old);
                    
                    $stmt_update = mysqli_prepare($conn, "UPDATE utilisateurs SET role = ? WHERE id = ?");
                    mysqli_stmt_bind_param($stmt_update, "si", $nouveau_role, $demande['id_utilisateur']);
        ...
        $stmt_update = mysqli_prepare($conn, "UPDATE demandes_acces SET statut = ?, id_admin_approbateur = ?, date_traitement = NOW(), commentaire_admin = ? WHERE id = ?");
        mysqli_stmt_bind_param($stmt_update, "siss", $nouveau_statut, $id_utilisateur, $commentaire, $id_demande);
...
if ($peut_demander) {
    ...
    mysqli_stmt_execute($stmt);
    $mes_demandes = mysqli_stmt_get_result($stmt);
    mysqli_stmt_close($stmt);
} else {
    // Admin : voir toutes les demandes en attente
    $demandes_en_attente = mysqli_query($conn, "
    ...
    $toutes_demandes = mysqli_query($conn, "
    ...
    <div class="content-wrapper">
        ...
        <?php if ($peut_demander): ?>
            ...
            <table>
                ...
                <tbody>
                    <?php if ($mes_demandes && mysqli_num_rows($mes_demandes) > 0): ?>
                        <?php while ($d = mysqli_fetch_assoc($mes_demandes)): ?>
                        <tr>
        ...
        <?php else: ?>
            ...
            <?php if ($demandes_en_attente && mysqli_num_rows($demandes_en_attente) > 0): ?>
                <div style="background: #f8f9fa; padding: 15px; border-radius: 10px; margin-bottom: 20px; border-left: 4px solid #dc3545;">
                    <strong style="color: #dc3545; font-size: 1.1em;">
                        📊 <?= mysqli_num_rows($demandes_en_attente) ?> demande<?= mysqli_num_rows($demandes_en_attente) > 1 ? 's' : '' ?> nécessite<?= mysqli_num_rows($demandes_en_attente) > 1 ? 'nt' : '' ?> votre attention
                    </strong>
                </div>
                <div style="display: grid; gap: 20px;">
                    <?php while ($d = mysqli_fetch_assoc($demandes_en_attente)): ?>
            ...
            <table>
                ...
                <tbody>
                    <?php if ($toutes_demandes && mysqli_num_rows($toutes_demandes) > 0): ?>
                        <?php while ($d = mysqli_fetch_assoc($toutes_demandes)): ?>
                        <tr>
                            <td><?= date('d/m/Y H:i', strtotime($d['date_demande'])) ?></td>
                            <td>
